...
struct WasmEdge::AOT::Compiler::CompileContext {
  ...
  bool SupportSSE4_1 = true; // NOLINT(readability-identifier-naming)
#else
  bool SupportSSE4_1 = false; // NOLINT(readability-identifier-naming)
#endif
...
Expect<void> Compiler::compile(Span<const Byte> Data, const AST::Module &Module,
  ...
  llvm::Triple Triple(LLModule.getTargetTriple());
  {
    ...
    if (!TheTarget) {
      // TODO:return error
      spdlog::error("loopTarget failed:{}", Error);
      return Unexpect(ErrCode::IllegalPath);
    }
